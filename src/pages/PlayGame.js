...
function PlayGame() {
  ...
  const playGame = (choice) => {
    ...
    opponentChoice().then((opponentChoice) =>
      declareWinner(choice, opponentChoice)
    );
  };
  ...
  return (
    <main className="pt-8">
      <div className="flex justify-between max-w-[700px] mx-8 md:mx-auto border-2 border-header-outline rounded-sm sm:rounded-2xl">
        <img
          className="mx-6 lg:mx-8 my-6 w-[83px] sm:w-auto self-center"
          src={images.logo}
          alt="rock paper scissors logo"
        />
        <div className="bg-white flex flex-col rounded-lg my-4 mx-6">
          <div className="px-6 py-2 sm:px-8 sm:py-4 text-center">
            <p className="uppercase text-score-text text-xs sm:text-base font-semibold tracking-[2.5px]">
              Score
            </p>
            <p
              id="scoreboard"
              className="text-4xl sm:text-6xl font-bold text-score-count"
            >
              {score}
            </p>
          </div>
      ...
      {!playerChoice ? (
        ...
        <div className="m-4 lg:m-0 flex justify-center items-center">
          ...
          {result && (
            <motion.div
              className="absolute lg:static text-center bottom-[15%]"
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              transition={{ ease: "easeOut", duration: 0.15 }}
            >
              <p className="mb-4 text-5xl sm:text-6xl font-bold text-center text-white uppercase">
                {result === "win"
                  ? "You Win"
                  : result === "lose"
                  ? "You Lose"
              ...
              <button
                onClick={() => resetGame()}
                className="px-14 py-3 bg-white text-dark-text rounded-lg cursor-pointer hover:text-[#DB2E4D] focus:text-[#DB2E4D] transition-all sm:text-base font-semibold uppercase tracking-[2.5px]"
              >
                Play Again
              </button>
            </motion.div>
...
const GameToken = ({ icon, onClick }) => {
  return (
    <button
      onClick={onClick}
      className="w-[130px] sm:w-[198px] cursor-pointer rounded-full transition-all focus:shadow-[0_0_0_14px_rgba(255,255,255,0.05)] sm:focus:shadow-[0_0_0_28px_rgba(255,255,255,0.05)] hover:shadow-[0_0_0_14px_rgba(255,255,255,0.05)] sm:hover:shadow-[0_0_0_28px_rgba(255,255,255,0.05)]"
    >
  ...
  return (
    <div className="sm:mx-14 flex flex-col-reverse lg:flex-col justify-between text-center">
      ...
      <motion.div
        ...
        animate={
          outcome === "win"
            ? { boxShadow: "0 0 0 42px rgba(255,255,255,0.05)" }
            : false
        }
        transition={{
          ease: "linear",
      ...
      >
        <img
          className="w-max-[130px] sm:w-max-[198px] rounded-full self-center"
          src={!playerChoice ? images.circle : images[playerChoice]}
          alt="game token"
        />
      </motion.div>
    </div>
